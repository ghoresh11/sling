...
def run():
    parser = argparse.ArgumentParser(
        ...
        usage = 'sling run [options] <run_id> <input_dir> <hmm_db> <req>')
    ...
    parser.add_argument('-fs','--fasta_suffix', type=str, help='Suffix of FASTA files in <input_dir>, defaults to all files', metavar='STR', default=".fasta")
    parser.add_argument('-gs','--gff_suffix', type=str, help='Suffix of GFF files in <gff_dir>, defaults to all files', metavar='STR', default=".gff")
    ...
    parser.add_argument('-di','--domains_to_ignore', type=str, help='File with line delemited hmmer domains to ignore in search', default = "", metavar="FILE")
    parser.add_argument('-mhs','--min_hmmscan_score', type=float, help='Minimum hmmer score to for significant hits [%(default)s]', default=20, metavar='FLOAT')
    parser.add_argument('-Mda','--max_diff_avg_length', type=int, help='Maximum difference between hit length and its average length in <domains_file> [%(default)s]', metavar='INT', default=None)
    parser.add_argument('-mhl','--min_hit_length', type=int, help='Minimum length of a hit, if not in <domains_file> [%(default)s]', metavar='INT', default=None)
    parser.add_argument('-Mhl','--max_hit_length', type=int, help='Maximum length of a hit, if not in <domains_file> [%(default)s]', metavar='INT', default=None)
    ...
    parser.add_argument('-t','--order', type=str, help='Order of partner genes in the operon. Options: upstream, downstream, either, three', metavar='STR', default=None)  
    ...
    parser.add_argument('-mbe','--min_blast_evalue', type=int, help='Minimun BLAST evalue to use for an edge in the sequence similarity network [%(default)s]', metavar='INT', default=0.01)
    parser.add_argument('-mi','--min_identity', type=int, help='Minimun BLAST identity to use for an edge in the sequence similarity network [%(default)s]', metavar='INT', default=30)
    ...
    parser.add_argument('req', help='Name of the predefined requirement file OR a path to a custom requirement file', metavar='STR/FILE')
